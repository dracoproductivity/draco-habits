...
export const useCloudSync = () => {
  ...
  const loadFromCloud = useCallback(async (userId: string) => {
    ...
    try {
      ...
      // Load draco state
      const { data: dracoData } = await supabase.from("draco_state").select("*").eq("user_id", userId).maybeSingle();
      ...
      // Load habit checks
      const { data: checksData } = await supabase.from("habit_checks").select("*").eq("user_id", userId);
      ...
      // Load daily logs
      const { data: logsData } = await supabase.from("daily_logs").select("*").eq("user_id", userId);
  ...
  const saveDraco = useCallback(
    async (dracoData: Partial<DracoState>) => {
      const userId = userIdRef.current || user?.id;
      if (!userId) {
  ...
  const saveSettings = useCallback(
    async (settingsData: Partial<AppSettings>) => {
  ...
  const deleteHabit = useCallback(
    async (habitId: string) => {
      ...
      await supabase.from("habit_checks").delete().eq("habit_id", habitId).eq("user_id", userId);
  ...
  const deleteCustomCategory = useCallback(
    async (categoryId: string) => {
      ...
      const { error } = await supabase.from("custom_categories").delete().eq("id", categoryId).eq("user_id", userId);
    ...
    const unsubscribe = useAppStore.subscribe((state, prevState) => {
      ...
      if (state.habits !== prevState.habits) {
        ...
        if (currentHabitsStr !== globalState.lastSyncedData.habits) {
          const previousHabits: Habit[] = globalState.lastSyncedData.habits
            ? JSON.parse(globalState.lastSyncedData.habits)
            : [];
      ...
      if (state.goals !== prevState.goals) {
        ...
        if (currentGoalsStr !== globalState.lastSyncedData.goals) {
          const previousGoals: Goal[] = globalState.lastSyncedData.goals
            ? JSON.parse(globalState.lastSyncedData.goals)
            : [];
      ...
      if (state.habitChecks !== prevState.habitChecks) {
        ...
        if (currentChecksStr !== globalState.lastSyncedData.habitChecks) {
          const previousChecks: HabitCheck[] = globalState.lastSyncedData.habitChecks
            ? JSON.parse(globalState.lastSyncedData.habitChecks)
            : [];
      ...
      if (state.customCategories !== prevState.customCategories) {
        ...
        if (currentCategoriesStr !== globalState.lastSyncedData.customCategories) {
          const previousCategories: CustomCategory[] = globalState.lastSyncedData.customCategories
            ? JSON.parse(globalState.lastSyncedData.customCategories)
            : [];
